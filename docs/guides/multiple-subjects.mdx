...
<Tabs groupId="language">
  ...
  ```typescript
  const newSession = await service.newSessionFromSession(previousSession);
  ```
  </TabItem>
  <TabItem value="python" label="Python">
  ```python
  new_session = service.new_session_from_session(previous_session)
  ```
  </TabItem>
</Tabs>
...
<Tabs groupId="language">
  ...
  repeat {
      ...
      try await service.calibrateSubject(subject, in: session, statusUpdate: callback)
  ...
  for (;;) {
    ...
    await service.calibrateSubject(subject, session, callback);
  ...
  while True:
      ...
      service.calibrate_subject(subject, session, callback)
