...
<Tabs groupId="language">
  ...
  if analysis_status["type"] == "completed":
      print("Analysis complete!")
  elif analysis_status["type"] == "failed":
      print("Analysis failed")
  ```
  </TabItem>
</Tabs>
...
<Tabs groupId="language">
  ...
  if analysis_status["type"] == "completed":
      results = service.download_trial_analysis_result_data(activity, ["metrics"])
...
<Tabs groupId="language">
  ...
  activities = service.get_activities_for_subject(
      subjects[0]["id"], start_index=0, count=10, sort="updated_at"
  )
  activity = activities[0]

  # Wait for processing
  activity_status = service.get_status(activity)
  while activity_status["type"] in ("uploading", "processing"):
      time.sleep(10)
      activity_status = service.get_status(activity)

  if activity_status["type"] != "ready":
      print("Activity failed processing")
  else:
      # Start analysis
