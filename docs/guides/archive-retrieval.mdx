...
<Tabs groupId="language">
  ...
  ```typescript
  const archive = await client.prepareArchive(session, true);
  ```
  </TabItem>
  <TabItem value="python" label="Python">
  ```python
  archive = client.prepare_archive(session)
  print("Archive task started:", archive.id)
  ```
  ...
  ```python
  archive = client.prepare_archive(session, with_videos=True)
  ```
  </TabItem>
</Tabs>
...
<Tabs groupId="language">
  ...
  ```python
  if status == ArchiveStatus.ready:
      zip_data = client.archive_data(archive)
...
<Tabs groupId="language">
  ...
  while (archiveStatus.type === "processing") {
    await new Promise(resolve => setTimeout(resolve, 20000));
    archiveStatus = await client.archiveStatus(archive);
  }
  ...
  while archive_status == ArchiveStatus.processing:
      time.sleep(20)
      archive_status = client.archive_status(archive)
