...
import checkerboardLight from '../../static/img/checkerboard-45.light.png';
import checkerboardDark from '../../static/img/checkerboard-45.dark.png';
...
<ThemedImage
  alt="Checkerboard corner counting diagram"
  sources={{ light: checkerboardLight, dark: checkerboardDark }}
  width="400"
/>
...
<Tabs groupId="language">
  ...
  <TabItem value="python" label="Python">
  ```python
  from modelhealth import CalibrationStatus, CalibrationStatusUploading, CalibrationStatusProcessing
  ...
  def on_status(status):
      if isinstance(status, CalibrationStatusUploading):
          ...
          if status.percent is not None:
              print(f"Processing: {status.percent}%")
          else:
              print("Processing...")
  ...
  service.calibrate_camera(session, details, on_status)
  ```
  </TabItem>
</Tabs>
