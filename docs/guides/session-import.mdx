...
See [`examples/scripts/python`](https://github.com/model-health/model-health/tree/main/examples/scripts/python) for a complete end-to-end script (`opencap_import.py`) that covers fetching trial data from OpenCap, subject selection, session configuration, per-trial activity type assignment and polling for analysis results.
:::
...
<Tabs groupId="language">
  ...
  session = client.import_session(activities_json, subject, session_config=config)
  ```
  </TabItem>
</Tabs>
