...
<img src={threeDViewExample} alt="3D View screen in the iOS demo app, showing a reconstruction of a subject performing an activity, with playback controls" width="200" style={{ flexShrink: 0, border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' }} />
</div>
...
If something outside `View3D` already drives playback position — for example a video player
you're keeping in sync — hand that clock full control instead of letting `View3D` run its own
timer:

- **Swift** — set `View3DController.setExternalClock(true)`. While enabled
  (`isExternalClockEnabled`), `play()` is a no-op and playback never advances on its own; call
  `seek(to:)` on every tick of your external clock instead. `seek(to:)` internally coalesces rapid
  calls, so it's safe to call at native video frame rate (30–60Hz).
- **TypeScript** — pass `externalClock` to `<View3D>`. While set, `play()` is a no-op and playback
  never advances on its own; call `seek(time)` on every tick of your external clock instead.
...
<img src={threeDViewOverlayExample} alt="3D View in the iOS demo app, showing blue and green ground reaction force vectors overlaid on the skeleton" width="200" style={{ flexShrink: 0, border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' }} />
</div>
