...
Before starting a recording, you can check whether your account is currently allowed to record — for example if you've reached your plan's activity limit for the period, or your subscription has lapsed.
...
<Tabs groupId="language">
  ...
  guard usage.recordingAllowed else {
      print("Recording blocked: \(usage.reason)")
      return
  }
  ```
  </TabItem>
  ...
  if not usage.recording_allowed:
      print(f"Recording blocked: {usage.reason}")
      return
  ```
  </TabItem>
</Tabs>
...
<Tabs groupId="language">
  <TabItem value="swift" label="Swift">
  ```swift
  let activity = try await client.startRecording(
      activityNamed: "cmj",
      in: session,
  ...
  const activity = await client.startRecording(
    "cmj",
    session,
...
<Tabs groupId="language">
  ...
  while (status.type === "uploading" || status.type === "processing") {
    ...
    status = await client.activityStatus(activity);
  }
...
<Tabs groupId="language">
  ...
  let resultData = await client.motionData(
  ...
  // Download raw videos
  let videos = await client.videos(for: activity, version: .raw)
  ...
  ```typescript
  // Download kinematics (MOT format) and marker trajectories (TRC format)
  const resultData = await client.motionDataForActivity(
    activity,
  ...
  // Download raw videos
  const videos = await client.videosForActivity(activity, "raw");
  ...
  ```python
  # Download kinematics (MOT format) and marker trajectories (TRC format)
  results = client.motion_data_for_activity(
  ...
  # Download raw videos
  videos = client.videos_for_activity(activity, "raw")
...
If you have another subject to capture with the same camera setup, see [Recording Multiple Subjects](./multiple-subjects) instead of repeating camera calibration.
