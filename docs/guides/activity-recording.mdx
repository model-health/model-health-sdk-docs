...
### Recording with Automatic Analysis

Pass an activity type to trigger analysis automatically once the recording is processed, without a separate `startAnalysis` call.
...
When an activity type is set, the activity status will transition to `.analyzing` after processing completes instead of `.ready`. See [Checking Processing Status](#checking-processing-status) below for how to handle this state.
