...
3. Select version **${VERSION}** (current beta release)
4. Add `ModelHealth` to your target

Alternatively, add it to your `Package.swift`:
```swift
dependencies: [
    .package(url: "https://github.com/model-health/model-health-swift", from: "${VERSION}")
]
```
...
### npm
```bash
npm install @modelhealth/modelhealth@${VERSION}
```
