...
> **Python note:** Camera calibration, neutral pose calibration, and recording require a connected mobile device and are handled by the Model Health web app, or the SDK's other supported langauages. The Python SDK is intended for post-capture workflows: listing sessions and subjects, running analysis on existing activities, and downloading results.
...
<Tabs groupId="language">
  ...
  if case .ready = status {
      ...
      if case .completed = analysisStatus {
          let results = await service.analysisResultData(
  ...
  if (subjects.length > 0) {
    await service.calibrateNeutralPose(
      subjects[0],
      session,
      (status) => {
  ...
  if (activityStatus.type === "ready") {
    ...
    if (analysisStatus.type === "completed") {
      const results = await service.downloadActivityAnalysisResultData(
        activity,
        ["metrics", "report"]
      );
  ...
  # List sessions recorded via the Model Health mobile app
  sessions = service.session_list()
  session = sessions[0]  # use the most recent session

  # List subjects
  subjects = service.subject_list()
  subject = subjects[0]

  # List activities for this subject
  activities = service.get_activities_for_subject(
      subject["id"], start_index=0, count=10, sort="updated_at"
  )
  activity = activities[0]  # use the most recent activity
  ...
  if status["type"] == "ready":
      ...
      # Poll for completion
      analysis_status = service.get_analysis_status(task)
      while analysis_status["type"] == "processing":
          time.sleep(10)
          analysis_status = service.get_analysis_status(task)

      # Download results
      if analysis_status["type"] == "completed":
          results = service.download_trial_analysis_result_data(
              activity, ["metrics", "report"]
          )
          for result in results:
              if result["result_data_type"] == "metrics":
                  metrics = json.loads(result["data"])
